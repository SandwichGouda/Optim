...
The references for this chapter are : \cite{charonhudry2019}, \cite{gowers2023}, \cite{gowers2024}, \cite{hudry2024}, besides thoses cited herein.
...
\begin{definition} Let $\Pi$ be a problem, let $I$ be an arbitrary instance of $\Pi$, of size $|I|$, and let $A$ be an algorithm solving $\Pi$. When we apply $A$ to $I$, $A$ performs a certain number of elementary operations $\psi_A(I)$. The (worst-case) complexity of $A$ is the function $\gamma_A$ defined from $\mathbf{N}$ to $\mathbf{N}$ by:
\[
...
\subsection{Polynomial and non-deterministic polynomial problems}
