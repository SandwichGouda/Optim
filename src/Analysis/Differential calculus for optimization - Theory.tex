...
\section{General definitions and results}

\begin{proposition}[First order characterization of Gradient Lipschitz Continuity]
    A differentiable function $ f : \Omega \to \mathbf R^n $ has $ L $-Lipschitz continuous gradient if and only if
    \[
        \forall x,y \in \Omega, \left| f(y) - f(x) - \langle \Delta f(x), y-x \rangle \right| \leqslant \frac L 2 \| x - y \|^2
    \]
\end{proposition}
...
\begin{proof}
    We have
    \begin{align*}
        \left| f(y) - f(x) - \langle \nabla(f(x)), y-x \rangle \right| &\leqslant \int_0^1 \left| \langle \nabla f(x + t(y-x)) - \nabla f(x), y-x \rangle \right| \mathrm dt \\
        &\leqslant \int_0^1 \| \nabla f(x + t(y-x)) - \nabla f(x) \| \| y-x \| \mathrm dt \\
        &\leqslant \| y-x \| \int_0^1 L \| t(y-x) \| \\
        &= \frac L 2 \| y-x \|^2
    \end{align*}
\end{proof}
