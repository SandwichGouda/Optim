...
\begin{example}\label{ex:add-xzero-to-neg-only}
    ...
    \[
        ...
        \right.
    \]
    Some sources explain that the auxiliary problem is crafted by adding $x_0$ on each line of the right-hand side, but there is a variant that consists in adding only a $x_0$ to the lines that have a negative right-hand side (i.e., $b_i$). We choose to illustrate using this variant. 
