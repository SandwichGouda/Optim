...
\begin{remark}
    Note that the constraints are nonstrict inequalities. Allowing strict inequalities would yield the constraints polyhedron to be non-closed, hence non-compact, and as such, there could be no solution. One can have a small think and observe that it is less interesting to optimize on non-closed domains, to prevent the case where no optimizer exists.
\end{remark}
